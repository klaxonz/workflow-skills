...
**不只是过 checklist——发挥你审查代码的能力。** 逐文件阅读，理解代码意图、数据流、边界条件，发现任何值得关注的问题。以下是常见检查起点，供参考而非限制：

- 能用 rg 辅助搜索的先搜，不能的逐文件阅读
- 对每个文件问自己：这段代码可能出错的地方在哪？设计上有没有更好的做法？未来维护者会不会困惑？
- 检查项不限于以下列表——发现任何问题都应记录
...
> 以上为常见检查项示例。**不要被它限制**——任何代码中的设计缺陷、逻辑问题、潜在 bug、可维护性问题都应该记录下来。
...
| **用户决策** | 让用户选创建哪些 issue |
| **减少噪音** | 同类合并 |
| **发挥判断** | 检查项为参考起点，不过滤问题——任何值得关注的设计缺陷、逻辑错误、可维护性问题都应记录 |
